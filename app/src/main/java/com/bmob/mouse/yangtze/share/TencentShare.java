 ...
 * @date 2014-3-3 TODO 实现QQ好友以及Qzone分享一键完成 usage:
 * 1.按opensdk说明文档配置好AndroidManifest.xml 
 * 2.在@see{TencentShareConstants}中填写相应分享内容,当调用无参数分享方法时，刚采用该接口中默认填写内容，当调用有参数分享时，刚采用传入参数 
 * 3.调用示例： TencentShare ts = new TencentShare(Activity,TencentShareEntity);
...
public class TencentShare implements TencentShareConstants {
    ...
    public TencentShare(Activity context, TencentShareEntity entity) {
        ...
        sp=context.getSharedPreferences(Constant.PRE_NAME, Context.MODE_PRIVATE);
        editor = sp.edit();
    }
        ...
        public void onComplete(Object arg0) {
            ...
            switch(flag) {
                case 0:
                    ...
                    break;
                case 1:
                    Log.i(TAG, "share to qzone complete!");
                    onShareToQZoneComplete();
                    break;
                case 2:
                    Log.i(TAG, "login complete and begin to story!");
